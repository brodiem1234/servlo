...
export default function ContactPage({ searchParams }: ContactPageProps) {
  return (
    <main className="mx-auto max-w-3xl space-y-6 px-4 py-12 text-slate-900">
      ...
      {searchParams?.success === "true" ? (
        <p className="rounded bg-emerald-50 px-3 py-2 text-sm text-emerald-700">Thanks - we&apos;ll get back to you shortly.</p>
      ) : null}
      <form action={submitContact} className="space-y-3 rounded-xl border bg-white p-4 shadow-sm">
        ...
        <button type="submit" className="rounded bg-[#1e3a5f] px-4 py-2 text-sm text-white">Send Message</button>
      </form>
    </main>
  );
}
