    ...
    <main className="min-h-screen bg-slate-50 p-4 md:p-8">
      <section className="mx-auto max-w-5xl space-y-6">
        <div>
          ...
          <p className="text-sm text-slate-600">{client.full_name ?? client.email ?? "Client"}</p>
        </div>
          ...
          <div className="mt-3 space-y-2 text-sm">
            {(quotes ?? []).map((quote) => (
              <div key={quote.id} className="rounded border p-3">
                ...
                <p className="text-slate-600 capitalize">{quote.status ?? "draft"}</p>
                {(quote.status ?? "").toLowerCase() !== "accepted" ? (
                  <div className="mt-2 flex gap-2">
                    <form action={acceptQuoteAction}>
                      <input type="hidden" name="quote_id" value={quote.id} />
                      <input type="hidden" name="token" value={params.token} />
                      <button type="submit" className="rounded bg-green-600 px-3 py-1 text-xs text-white">Accept</button>
                    </form>
                    <form action={declineQuoteAction}>
                      <input type="hidden" name="quote_id" value={quote.id} />
                      <input type="hidden" name="token" value={params.token} />
                      <button type="submit" className="rounded bg-red-600 px-3 py-1 text-xs text-white">Decline</button>
                    </form>
                  </div>
                ) : null}
              </div>
            ))}
            {(quotes ?? []).length === 0 ? <p className="text-slate-500">No quotes available.</p> : null}
          </div>
        </article>
              ...
              .map((invoice) => (
                <div key={invoice.id} className="flex items-center justify-between rounded border p-3">
                  <div>
                    ...
                    <p className="text-slate-600">${Number(invoice.amount ?? 0).toFixed(2)}</p>
                  </div>
                  <a
                    href={process.env.NEXT_PUBLIC_STRIPE_PAYMENT_LINK || "#"}
                    className="rounded bg-[#1e3a5f] px-3 py-1 text-xs text-white"
                  >
                    Pay Now
                  </a>
                </div>
