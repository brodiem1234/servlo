...
export function AccentInlineScript({ accentHex }: Props) {
  ...
  const js = `(function(){try{var KEY=${keyJson};var LEGACY=${legacyKeyJson};var FALLBACK=${fallbackJson};var r=document.documentElement;var cand=null;try{cand=localStorage.getItem(KEY);}catch(e){}if(!cand){try{cand=localStorage.getItem(LEGACY);}catch(e){}}function hover(h){return "color-mix(in srgb, "+h+" 82%, black)";}function ok(h){return typeof h==="string"&&/^#[0-9A-Fa-f]{6}$/.test(h.trim());}var h=ok(cand)?cand.trim().toUpperCase():FALLBACK;r.style.setProperty("--brand-accent",h);r.style.setProperty("--brand-accent-hover",hover(h));}catch(e){}})();`;
